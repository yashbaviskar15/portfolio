...
export const TopBar: React.FC<TopBarProps> = ({ onToggleActivities, isActivitiesOpen, onOpenApp }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !document.getElementById('quick-settings-popover')?.contains(target)
      ) {
      ...
      if (
        ...
        !document.getElementById('calendar-popover')?.contains(target)
      ) {
      ...
      if (
        ...
        !document.getElementById('help-menu-popover')?.contains(target)
      ) {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 h-8 select-none z-50 flex items-center justify-between px-2 sm:px-3 text-xs font-medium border-b transition-colors duration-150 ${
          themeMode === 'dark'
            ? 'bg-[#121214]/90 text-neutral-200 border-white/10'
            : 'bg-[#f4f4f5]/90 text-neutral-800 border-neutral-300 shadow-xs'
        } backdrop-blur-xl`}
      >
        {/* Left: Activities Button + Help Menu */}
        <div className="flex items-center gap-1 sm:gap-1.5">
          <button
            type="button"
            onClick={onToggleActivities}
            className={`px-2.5 sm:px-3 py-0.5 rounded-full text-[11.5px] sm:text-xs font-semibold tracking-wide transition-all cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 ${
              isActivitiesOpen
        ...
        {/* Center: Date & Time Clock Trigger */}
        <div ref={calendarRef}>
          <button
            type="button"
            onClick={() => {
            ...
            className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-0.5 rounded-full transition-all cursor-pointer text-[11px] sm:text-xs font-medium tracking-tight focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 ${
              calendarOpen
          ...
          >
            <span className="truncate max-w-[105px] sm:max-w-none">{timeString || 'Aug 30 10:44 AM'}</span>
        ...
        {/* Right: System Indicators Pill */}
        <div className="flex items-center gap-1 sm:gap-1.5 relative" ref={quickSettingsRef}>
          {/* Compact "EN" language indicator */}
          <button
            type="button"
          ...
          <button
            ...
            className={`flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-3 py-0.5 rounded-full transition-all cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 ${
              quickSettingsOpen
                ? 'bg-white/20 font-semibold'
                : themeMode === 'dark'
                ? 'text-neutral-200 hover:bg-white/10'
          ...
          >
            ...
            <div className="flex items-center gap-0.5 sm:gap-1">
              <span className="hidden sm:inline text-[10px] sm:text-[10.5px] font-mono">100%</span>
              <motion.div
        ...
        <AnimatePresence>
          {helpMenuOpen && (
            <motion.div
              id="help-menu-popover"
              variants={dropdownMenuVariants}
              initial="hidden"
              animate="visible"
              exit="exit"
              className={`fixed top-9 left-2 sm:left-3 w-48 rounded-xl border shadow-2xl backdrop-blur-2xl p-1.5 z-50 space-y-0.5 ${
                themeMode === 'dark'
        ...
        {/* 2. GNOME Calendar & Notification Center Popover (Perfect Viewport Centering) */}
        <AnimatePresence>
          {calendarOpen && (
            <motion.div
              id="calendar-popover"
        ...
        {/* 3. GNOME Quick Settings Popover (Viewport Right Pinned) */}
        <AnimatePresence>
          {quickSettingsOpen && (
            <motion.div
